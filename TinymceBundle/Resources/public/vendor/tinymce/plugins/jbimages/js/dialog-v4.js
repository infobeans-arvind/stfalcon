...
var jbImagesDialog = {
	...
	inProgress : function() {
		...
		this.timeoutStore = window.setTimeout(function(){
			document.getElementById("upload_additional_info").innerHTML = 'This is taking longer than usual.';
	...
	uploadFinish : function(result) {
		...
		else
		{
			...
			tinymce = w.tinymce;
                        ...
                        ext = result.filename.substring( result.filename.lastIndexOf('.')+1);
                        if (image_types.indexOf(ext.toLowerCase()) != -1)
                            tinymce.EditorManager.activeEditor.insertContent('<img src="' + result.filename +'">');
                        else {
                            if (result.client_name == '') {
                                tinymce.EditorManager.activeEditor.insertContent('<a href="' + result.filename +'">'+result.client_name+'</a>');
                            } else {
                                tinymce.EditorManager.activeEditor.insertContent('<a href="' + result.filename +'">'+result.client_name+'</a>');
                            }
                        }
